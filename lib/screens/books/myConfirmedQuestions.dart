...
class MyConfirmedQuestions extends StatefulWidget {
  var _userPassData = List.filled(2, '', growable: false);
  bookModel _book;
  MyConfirmedQuestions(this._userPassData, this._book);

  @override
  _MyConfirmedQuestionsState createState() => new _MyConfirmedQuestionsState(_userPassData, _book);
}

class _MyConfirmedQuestionsState extends State<MyConfirmedQuestions> {
  var _userPassData = List.filled(2, '', growable: false);
  bookModel _book;
  _MyConfirmedQuestionsState(this._userPassData, this._book);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text('سوالات تائید شده من', style: TextStyle(fontSize: 16, fontFamily: "Vazir"),),
        ...
        child: Stack(
            children: <Widget>[
              InAppWebView(
                ...
                initialUrlRequest:
                URLRequest(
                    url: WebUri.uri(Uri.parse( GlobalKeys.ebargeUrl + '/index.php?option=com_ebarge&view=pageqs&whichqs=mineconfirmed&tmpl=component&bookid=' +
                        _book.book_id!, ),),
                    method: "post",
