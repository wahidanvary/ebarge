...
class MyBookSarbarg extends StatefulWidget {
  var _userPassData = List.filled(2, '', growable: false);
  bookModel _book;
  MyBookSarbarg(this._userPassData, this._book);

  @override
  _MyBookSarbargState createState() => new _MyBookSarbargState(_userPassData, _book);
}

class _MyBookSarbargState extends State<MyBookSarbarg> {
  var _userPassData = List.filled(2, '', growable: false);
  bookModel _book;
  final GlobalKey webViewKey = GlobalKey();

  _MyBookSarbargState(this._userPassData, this._book);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text(
          'سربرگ های امتحانی کتاب', style: TextStyle(fontSize: 16, fontFamily: "Vazir"),),
        ...
        child: Stack(
            children: <Widget>[
              InAppWebView(
                ...
                initialUrlRequest:
                URLRequest(
                    url: WebUri.uri(Uri.parse(GlobalKeys.ebargeUrl + '/index.php?option=com_ebarge&view=ebuserschoolinfo&layout=edit&tmpl=component&bookid=' +
                        _book.book_id!),),
                    method: "post",
