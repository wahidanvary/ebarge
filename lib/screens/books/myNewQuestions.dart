...
class MyNewQuestions extends StatefulWidget {
  var _userPassData = List.filled(2, '', growable: false);
  bookModel _book;
  MyNewQuestions(this._userPassData, this._book);
  @override
  _MyNewQuestionsState createState() => new _MyNewQuestionsState(_userPassData, _book);
}

class _MyNewQuestionsState extends State<MyNewQuestions> {
  var _userPassData = List.filled(2, '', growable: false);
  bookModel _book;

  _MyNewQuestionsState(this._userPassData, this._book);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text('سوالات امتحانی جدید من', style: TextStyle(fontSize: 16, fontFamily: "Vazir"),),
        ...
        child: Stack(
            children: <Widget>[
              InAppWebView(
                ...
                initialUrlRequest:
                URLRequest(
                    url: WebUri.uri(Uri.parse( GlobalKeys.ebargeUrl + '/index.php?option=com_ebarge&view=co_qs_rejected&layout=mobile&whichqs=minenew&tmpl=component&bookid=' +
                        _book.book_id!,),),
                    method: "post",
