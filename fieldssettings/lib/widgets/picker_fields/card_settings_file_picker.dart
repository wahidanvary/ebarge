...
class CardSettingsFilePicker extends FormField<Uint8List>
  ...
  /// fires when the picker data is changed Wersa
  @override
  final ValueChanged<myIO.File?>? onChanged;
...
class _CardSettingsFilePickerState extends FormFieldState<Uint8List> {
    ...
    if (value == null) {
      showMyFilePicker(
        ...
        onChanged: (file) => setState(() {
          var uint8file = file.readAsBytesSync();
          didChange(uint8file);
  ...
  //Wersa
  Future<void> showMyFilePicker({
    FileType fileType = FileType.image,
    required ValueChanged<myIO.File> onChanged,
  }) async {
    try {
      myIO.File rawVideoFile;
      filePicker.FilePickerResult? platformFile = await filePicker.FilePicker.platform.pickFiles(type: widget.fileType!);

      if(platformFile != null) {
         rawVideoFile = myIO.File(platformFile.files.single.path!);;
      } else return;
      if (onChanged != null && rawVideoFile != null) onChanged(rawVideoFile);
    } catch (error) {
