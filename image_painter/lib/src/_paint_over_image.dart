...
///
class ImagePainterState extends State<ImagePainter>  with DiagnosticableTreeMixin {
                    ...
                    return ImagePainterTransformer(
                      ...
                      child: CustomPaint(
                        ...
                        painter: DrawImage(
                          image: _image,
                          points: _points,
                          paintHistory: context.read<HistoryModel>().paintHistory,
                          isDragging: _inDrag,
                          update: UpdatePoints(
                              start: _start,
                              end: _end,
              ...
              return ImagePainterTransformer(
                ...
                child: CustomPaint(
                  ...
                  painter: DrawImage(
                    ...
                    points: _points,
                    paintHistory: context.watch<HistoryModel>().paintHistory,
                    isDragging: _inDrag,
                    update: UpdatePoints(
                        start: _start,
                        end: _end,
    ...
    setState(
      () {
        ...
        if (ctrl.mode == PaintMode.text &&
            context.read<HistoryModel>().paintHistory!
  ...
  Future<ui.Image> _renderImage() async {
    ...
    final painter = DrawImage(image: _image, paintHistory: context.read<HistoryModel>().paintHistory);
  ...
  Future<Uint8List?> exportImage() async {
    ...
    } else if (widget.byteArray != null && context.watch<HistoryModel>().paintHistory!.isEmpty) {
  ...
  Future<void> _openTextDialog() async{
    ...
    var _xPosition = _image!.width.toDouble() / 2;
    var _yPosition = _image!.height.toDouble() / 2;

    List<Offset>? _textOffset = <Offset>[];
    _textOffset.add(Offset(_xPosition, _yPosition));
    ...
    if (_textController.text != '') {
      context.read<HistoryModel>().openTextDialog(_textController.text , _painter, _textOffset);
 ...
 /* void _openTextDialog() {
   ...
   // final context = _repaintKey.currentContext;
    ...
    TextDialog.show(context, _textController, fontSize, _controller.value.color,
      ...
      if (_textController.text != '') {
        setState(() {
          context.read<HistoryModel>().paintHistory!.add(
            PaintInfo(
                mode: PaintMode.text,
    ...
    return Container(
      ...
      child: Row(
        ...
        /*  IconButton(
              tooltip: "ذخیره طراحی",
              icon:
              widget.undoIcon ?? Icon(Icons.save, color: Colors.grey[700]),
              onPressed: () {
                if (context.read<HistoryModel>().paintHistory!.isNotEmpty) {
                  setState(context.read<HistoryModel>().paintHistory!.removeLast);
                }
              }),*/
          Transform(
          ...
          IconButton(
            tooltip: "پاک کردن طراحی ها",
            icon: widget.clearAllIcon ??
                Icon(Icons.clear, color: Colors.grey[700]),
            onPressed: () => setState(context.read<HistoryModel>().paintHistory!.clear),
          ),
        ],
      ),
    );
